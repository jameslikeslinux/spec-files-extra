#
# spec file for package SFEghc-uu-parsinglib
#
...
Name:                    uu-parsinglib
Summary:                 uu-parsinglib - New version of the Utrecht University parser combinator library
Version:                 2.3.4
Release:                 1
License:                 LGPL
...
%description
uu-parsinglib-2.3.1 @ hackage.haskell.org

New version of the Utrecht University parser combinator library,
providing online, error correction, annotation free, applicative style
parser combinators which also provides a monadic interface

The file Text.ParserCombinators.UU.Examples contains a ready-made main
function, which can be called to see the error correction at work.
...
Versions above 2.3 . contain some form of abstract interpretation from
the old uulib versions of these combinators . are closer to Haskell98,
since they do not make use of type families anymore . note that the
basic parser interface will probably not change when we add more
features, but the calling conventions of the outer parser and the
class structure upon which the parametrisation is based may change

Version 2.3.1 . fix for GHC 6.12, because of change in GADT definition
handling

%package -n SFEghc-uu-parsinglib-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-uu-parsinglib

%package -n SFEghc-uu-parsinglib-doc
...
%post -n SFEghc-uu-parsinglib-doc
...
%postun -n SFEghc-uu-parsinglib-doc
...
%files -n SFEghc-uu-parsinglib-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-uu-parsinglib-doc -f pkg-doc.files
%defattr(-,root,root,-)
...
- Bump from 2.3.1 to 2.3.4.
* Sun May 23 2010 - markwright@internode.on.net
- Initial Solaris version 2.3.1
