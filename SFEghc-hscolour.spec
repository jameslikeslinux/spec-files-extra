#
# spec file for package SFEghc-hscolour
#
...
Name:                    hscolour
Summary:                 hscolour - Colourise Haskell code
Version:                 1.17
Release:                 1
License:                 GPL
...
chmod a+x ./Setup.hs
./Setup.hs configure --prefix=%{_prefix} \
    --libdir=%{_cxx_libdir} \
...
export LD_OPTIONS='-L/usr/gnu/lib -R/usr/gnu/lib'
%endif
./Setup.hs build ${VERBOSE}
./Setup.hs haddock ${VERBOSE} --executables --hoogle
...
./Setup.hs register ${VERBOSE} --gen-pkg-config=%{name}-%{version}.conf
./Setup.hs copy ${VERBOSE} --destdir=${RPM_BUILD_ROOT}
...
%files -n SFEghc-hscolour-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-hscolour-doc -f pkg-doc.files
%defattr(-,root,root,-)
