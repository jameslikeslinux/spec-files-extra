...
Summary:                 hexpat-iteratee - Chunked XML parsing using iteratees
Version:                 0.6
Release:                 1
License:                 BSD
...
SUNW_Pkg:		 SFEghc-hexpat-iteratee
...
implemented using a special HandlerT monad. This monad is suspended
whenever it tries to read a part of the tree that hasn't been parsed
yet, and continued as soon as it is available. The resulting code
looks and functions very much as if you were using lazy I/O, only
without the associated problems. The handler monad may have effects.
...
%package -n SFEghc-hexpat-iteratee-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-hexpat-iteratee

%package -n SFEghc-hexpat-iteratee-doc
...
%post -n SFEghc-hexpat-iteratee-doc
...
%postun -n SFEghc-hexpat-iteratee-doc
...
%files -n SFEghc-hexpat-iteratee-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-hexpat-iteratee-doc -f pkg-doc.files
%defattr(-,root,root,-)
...
- Bump from 0.3 to 0.6.
* Thu May 06 2010 - markwright@internode.on.net
- Initial Solaris version 0.3
