#
# spec file for package SFEghc-unix-compat
#
...
Name:                    unix-compat
Summary:                 unix-compat - Portable POSIX-compatibility layer.
Version:                 0.1.2.1
Release:                 1
License:                 BSD
...
SUNW_Pkg:		 SFEghc-unix-compat
SUNW_BaseDir:            %{_basedir}
BuildRoot:               %{_tmppath}/%{name}-%{version}-build

Patch1:                  ghc-unix-compat-01-solaris.diff
...
it is not available, portable implementations are used.

%package -n SFEghc-unix-compat-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-unix-compat

%package -n SFEghc-unix-compat-doc
...
%postun -n SFEghc-unix-compat-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-unix-compat-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-unix-compat-doc -f pkg-doc.files
%defattr(-,root,root,-)
