#
# spec file for package SFEghc-executable-path
#
...
Summary:                 executable-path - Finding out the full path of the executable.
Version:                 0.0.1
Release:                 1
...
SUNW_Pkg:		 SFEghc-executable-path
...
%package -n SFEghc-executable-path-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-executable-path

%package -n SFEghc-executable-path-doc
...
sed -i -e 's,solaris_HOST_OS,solaris2_HOST_OS,' System/Environment/Executable.hs
chmod a+x ./Setup.lhs
...
%post -n SFEghc-executable-path-doc
...
%postun -n SFEghc-executable-path-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-executable-path-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-executable-path-doc -f pkg-doc.files
%defattr(-,root,root,-)
