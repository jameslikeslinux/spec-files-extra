#
# spec file for package SFEghc-io-storage
#
...
Name:                    io-storage
Summary:                 io-storage - A key-value store in the IO monad.
Version:                 0.3
Release:                 1
License:                 BSD
...
%package -n SFEghc-io-storage-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-io-storage

%package -n SFEghc-io-storage-doc
...
%postun -n SFEghc-io-storage-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-io-storage-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-io-storage-doc -f pkg-doc.files
%defattr(-,root,root,-)
