#
# spec file for package SFEghc-safe-failure
#
...
Name:                    safe-failure
Summary:                 safe-failure - Library for safe (pattern match free) functions
Version:                 0.5.0
Release:                 1
License:                 BSD
...
to fail in a Failure. These functions can be used to reduce the number
of unsafe pattern matches in your code.  These functions
can be used to reduce the number of unsafe pattern matches in your
code.

%package -n SFEghc-safe-failure-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-safe-failure

%package -n SFEghc-safe-failure-doc
...
%post -n SFEghc-safe-failure-doc
...
%postun -n SFEghc-safe-failure-doc
...
%files -n SFEghc-safe-failure-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-safe-failure-doc -f pkg-doc.files
%defattr(-,root,root,-)
