#
# spec file for package SFEghc-hxt
#
...
Name:                    hxt
Summary:                 hxt - A collection of tools for processing XML with Haskell.
Version:                 8.3.2
Release:                 1
License:                 Other
...
Requires: SFEghc-parallel.spec
Requires: SFEghc-tagsoup.spec

%description
The Haskell XML Toolbox bases on the ideas of HaXml and HXML, but introduces
a more general approach for processing XML with Haskell. The Haskell XML Toolbox
uses a generic data model for representing XML documents, including the DTD
subset and the document subset, in Haskell. It contains a validating XML parser,
a HTML parser, namespace support, an XPath expression evaluator, an XSLT
library, a RelaxNG schema validator and funtions for serialization and
deserialization of user defined data. The libraray make extensive use of the
arrow approach for processing XML.

%package -n SFEghc-hxt-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-hxt

%package -n SFEghc-hxt-doc
...
%files -n SFEghc-hxt-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-hxt-doc -f pkg-doc.files
%defattr(-,root,root,-)
