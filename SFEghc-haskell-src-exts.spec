#
# spec file for package SFEghc-haskell-src-exts
#
...
SUNW_Pkg:		 SFEghc-haskell-src-exts
...
%package -n SFEghc-haskell-src-exts-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-haskell-src-exts

%package -n SFEghc-haskell-src-exts-doc
...
%post -n SFEghc-haskell-src-exts-doc
...
%postun -n SFEghc-haskell-src-exts-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-haskell-src-exts-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-haskell-src-exts-doc -f pkg-doc.files
%defattr(-,root,root,-)
