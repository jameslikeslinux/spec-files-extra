#
# spec file for package SFEghc-vector-space
#
...
Name:                    vector-space
Summary:                 vector-space - Vector & affine spaces, linear maps, and derivatives (requires ghc 6.9 or better)
# SFEghc-vty wants 0.5.9
Version:                 0.5.9
Release:                 1
License:                 BSD
...
Requires: SFEghc-Boolean
Requires: SFEghc-MemoTrie
...
%package -n SFEghc-vector-space-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-vector-space

%package -n SFEghc-vector-space-doc
...
%post -n SFEghc-vector-space-doc
...
%postun -n SFEghc-vector-space-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-vector-space-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-vector-space-doc -f pkg-doc.files
%defattr(-,root,root,-)
