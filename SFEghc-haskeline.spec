#
# spec file for package SFEghc-haskeline
#
...
Name:                    haskeline
Summary:                 haskeline - A command-line interface for user input, written in Haskell.
Version:                 0.6.2.2
Release:                 1
License:                 BSD
...
%package -n SFEghc-haskeline-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-haskeline

%package -n SFEghc-haskeline-doc
...
# The current haskell-platform 2010.1.0.0 only has utf8-string 0.3.4
sed -i -e 's,utf8-string==0.3.* && >=0.3.6,utf8-string==0.3.* \&\& >=0.3.4,' haskeline.cabal
chmod a+x ./Setup.hs
...
%files -n SFEghc-haskeline-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-haskeline-doc -f pkg-doc.files
%defattr(-,root,root,-)
