...
# Mostly taken from the one in the sourcecode with some very small changes
# to make it compatible with SFE
#
...
# no no no no no can't do or one architecture gets delated before install :D %{__rm} -rf $RPM_BUILD_ROOT
