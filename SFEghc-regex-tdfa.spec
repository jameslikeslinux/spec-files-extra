#
# spec file for package SFEghc-regex-tdfa
#
...
Name:                    regex-tdfa
Summary:                 regex-tdfa - Replaces/Enhances Text.Regex
Version:                 1.1.2
Release:                 1
License:                 BSD
...
%package -n SFEghc-regex-tdfa-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-regex-tdfa

%package -n SFEghc-regex-tdfa-doc
...
%postun -n SFEghc-regex-tdfa-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-regex-tdfa-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-regex-tdfa-doc -f pkg-doc.files
%defattr(-,root,root,-)
