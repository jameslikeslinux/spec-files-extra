#
# spec file for package SFEghc-control-monad-attempt
#
...
Summary:                 control-monad-attempt - Monad transformer for attempt.
Version:                 0.3.0
Release:                 1
...
URL:                     http://github.com/pepeiborra/control-monad-attempt
Source:                  http://hackage.haskell.org/packages/archive/%{name}/%{version}/%{name}-%{version}.tar.gz
SUNW_Pkg:		 SFEghc-c-monad-attempt
...
Requires: SFEghc-attempt

%description
This package uses the transformers library.

%package -n SFEghc-c-monad-attempt-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-c-monad-attempt

%package -n SFEghc-c-monad-attempt-doc
...
%post -n SFEghc-c-monad-attempt-doc
...
%postun -n SFEghc-c-monad-attempt-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-c-monad-attempt-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-c-monad-attempt-doc -f pkg-doc.files
%defattr(-,root,root,-)
