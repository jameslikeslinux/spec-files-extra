#
# spec file for package SFEghc-monadloc
#
...
Name:                    monadloc
Summary:                 monadloc - A class for monads which can keep a monadic call trace
Version:                 0.5
Release:                 1
...
A preprocessor is provided which can insert calls to
Control.Monad.Loc.withLoc before every monadic statement in a
module. To invoke the preprocessor, add the pragma OPTIONS_GHC -F
-pgmF MonadLoc at the top of your Haskell files together with an
import for the Control.Monad.Loc module

This package provides no implementation of the
Control.Monad.Loc.MonadLoc interface. Currently the only package that
does so is control-monad-exception, but any other package can
implement it and provide monadic call traces.

%package -n SFEghc-monadloc-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-monadloc

%package -n SFEghc-monadloc-doc
...
%files -n SFEghc-monadloc-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-monadloc-doc -f pkg-doc.files
%defattr(-,root,root,-)
