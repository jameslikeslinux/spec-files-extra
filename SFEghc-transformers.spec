#
# spec file for package SFEghc-transformers
#
...
%package -n SFEghc-transformers-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-transformers

%package -n SFEghc-transformers-doc
...
%post -n SFEghc-transformers-doc
...
%postun -n SFEghc-transformers-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-transformers-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-transformers-doc -f pkg-doc.files
%defattr(-,root,root,-)
