#
# spec file for package SFEghc-fingertree
#
...
Name:                    fingertree
Summary:                 fingertree - Generic finger-tree structure, with example instances
Version:                 0.0.1.0
Release:                 1
License:                 BSD
...
%package -n SFEghc-fingertree-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-fingertree

%package -n SFEghc-fingertree-doc
...
%postun -n SFEghc-fingertree-doc
if [ "$1" -eq 0 ] ; then
  cd %{_docdir}/ghc/html/libraries && [ -x "./gen_contents_index" ] && ./gen_contents_index
fi
...
%files -n SFEghc-fingertree-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-fingertree-doc -f pkg-doc.files
%defattr(-,root,root,-)
