#
# spec file for package SFEghc-fclabels
#
...
Name:                    fclabels
Summary:                 fclabels - First class accessor labels
Version:                 0.4.2.1
Release:                 1
License:                 BSD
...
%description
First class labels that act as bidirectional record fields. The labels
are fully composable and can be used to get, set and modify parts of a
datatype in a consistent way. The label datatype, conveniently called
:->, is an instance of the Category type class meaning it has a proper
identity and composition. The library has support for automatically
deriving labels from record selectors that start with an
underscore. Labels can be used in a purely functional setting or be
applied to mutable state in some state monad.

%package -n SFEghc-fclabels-prof
Summary:                 %{summary} - profiling libraries
SUNW_BaseDir:            %{_basedir}
%include default-depend.inc
Requires: SFEghc-fclabels

%package -n SFEghc-fclabels-doc
...
%files -n SFEghc-fclabels-prof -f pkg-prof.files
%defattr (-, root, bin)

%files  -n SFEghc-fclabels-doc -f pkg-doc.files
%defattr(-,root,root,-)
...
- Initial Solaris version 0.4.2.1
